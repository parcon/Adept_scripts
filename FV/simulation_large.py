  ...
  def readJobs(self, filename):
	...
	REPLAYLIST = open(replayFile)
        ...
        for job in jobs: 
          ...
          (type, pick, drop, pickpri, droppri, echo_string) = job.split(" ", 5)
...
hostList = ["10.0.201.101", "10.0.201.102", "10.0.201.103", "10.0.201.104", "10.0.201.105", "10.0.201.106", "10.0.201.107", "10.0.201.108", "10.0.201.109"]
sim = SIM()
  ...
  if ( cim.currentReplayJob < cim.numReplayJobs ):
    if (( time.time() - cim.lastJobTime) >
      cim.rawReplayJobList[cim.currentReplayJob].interval and
       cim.numJobsPending < 50 ):  # the "100" used to be 4   can probably take that out
